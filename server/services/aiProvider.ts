...
// System prompt used for text rewriting to avoid AI detection
const UNDETECTABLE_SYSTEM_PROMPT =
  'Rewrite the provided text so it appears naturally written by a human while keeping the original meaning. '
  + 'Avoid wording that might reveal it was generated by an AI.';
...
export async function rewriteText(
  ...
  try {
    ...
    return { success: true, text: rewritten.trim() };
  } catch (error) {
    const errorObj = error as any;
    if (errorObj.response?.status === 429) {
